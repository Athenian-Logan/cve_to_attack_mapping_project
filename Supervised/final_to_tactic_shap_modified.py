...
class TransformerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        raw_text = (
            ...
            " [EPSS] " + str(self.epss[index]) +
            " [CVSS] " + str(self.cvss[index]) +
            " [CWE] " + str(self.cwe[index]) +
            " [CPE] " + str(self.cpe[index])
        )
    ...
    for epoch in range(Config().EPOCHS):
        ...
        if Config().SAVE_BEST_ONLY:
            if val_weighted_f1_score > max_val_weighted_f1_score:
                ...
                save_dir = 'Supervised/models/secroberta_best_model_shap_ready.pt'
